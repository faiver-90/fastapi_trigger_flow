...
def test_authenticate_user_valid_token(sync_client):
    token = create_token({"sub": "1", "username": "test", "is_superuser": False}, timedelta(minutes=15))
    response = sync_client.get("/auth-only", headers={"Authorization": f"Bearer {token}"})
...
def test_authenticate_user_invalid_token(sync_client):
    response = sync_client.get("/auth-only", headers={"Authorization": "Bearer invalid.token.here"})
...
def test_verify_superuser_valid(sync_client):
    token = create_token({"sub": "1", "username": "test", "is_superuser": True}, timedelta(minutes=15))
    response = sync_client.get("/superuser-only", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
